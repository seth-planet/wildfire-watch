...
class DockerContainerManager:
    ...
    def __init__(self, client: docker.DockerClient = None, worker_id: str = 'master'):
        ...
        self.created_images = []
        # Reference counting for shared containers
        self._container_refs = {}  # container_name -> reference_count
        self._container_lock = threading.Lock()  # Thread-safe access to refs
    
    def get_free_port(self) -> int:
        """Get a free port for container use.
    ...
    def cleanup_old_container(self, container_name: str, timeout: int = 5, retry_count: int = 3) -> None:
        ...
        """
        print(f"[Worker: {self.worker_id}] cleanup_old_container called for: {container_name}")
        
        # Check if container belongs to this worker
        if not container_name.startswith(self.container_prefix):
            print(f"  Container {container_name} belongs to different worker, skipping cleanup")
            return
            
        # Check reference count first
        with self._container_lock:
            if self._container_refs.get(container_name, 0) > 0:
                print(f"  Container {container_name} has active references ({self._container_refs.get(container_name, 0)}), skipping cleanup")
                return
        ...
        for attempt in range(retry_count):
            try:
                ...
                # Check container age - don't remove containers created less than 30 seconds ago
                labels = old_container.labels
                created_time = float(labels.get('com.wildfire.created', '0'))
                if created_time > 0:
                    age = time.time() - created_time
                    if age < 30:  # 30 second grace period
                        print(f"  Container {container_name} is too new ({age:.1f}s old), skipping cleanup")
                        return
    ...
    def build_image_if_needed(self, image_tag: str, dockerfile_path: str, 
                             build_context: str = None, force_rebuild: bool = False) -> None:
        """
        Build Docker image if it doesn't exist or if force_rebuild is True.
        
        Args:
            image_tag: Tag for the image
            dockerfile_path: Path to Dockerfile
            build_context: Build context directory (defaults to current directory)
            force_rebuild: Force rebuild even if image exists (useful for outdated images)
        """
        image_exists = False
        try:
            ...
            image_exists = True
            if force_rebuild:
                print(f"  Force rebuilding image: {image_tag} (existing image will be replaced)")
                # Remove the old image first to ensure clean rebuild
                try:
                    self.client.images.remove(image_tag, force=True)
                except Exception as e:
                    print(f"    Warning: Could not remove old image: {e}")
        except docker.errors.ImageNotFound:
            print(f"  Image not found, building: {image_tag}")
        ...
        if not image_exists or force_rebuild:
            ...
            try:
                ...
                image, build_logs = self.client.images.build(
                    ...
                    tag=image_tag,
                    rm=True,
                    nocache=force_rebuild  # Don't use cache if force rebuilding
                )
    ...
    def start_container(self, image: str, name: str, config: Dict[str, Any],
        ...
        with self._container_lock:
            ...
            try:
                ...
                if existing_container.status == 'running':
                    print(f"  Container {name} already running, incrementing reference count")
                    self._container_refs[name] = self._container_refs.get(name, 0) + 1
        ...
        try:
            ...
            print(f"[Worker: {self.worker_id}] Creating container {name} with image {image}")
            # Merge name into config
            config['name'] = name
            config['image'] = image
            
            # Add labels for cleanup identification and tracking
            config.setdefault('labels', {})
            config['labels']['com.wildfire.test'] = 'true'
            config['labels']['com.wildfire.worker'] = self.worker_id
            config['labels']['com.wildfire.created'] = str(time.time())
            ...
            print(f"[Worker: {self.worker_id}] Container config: {json.dumps({k: str(v) for k, v in config.items() if k not in ['environment']}, indent=2)}")
            
            container = self.client.containers.run(**config)
            self.created_containers.append(container)
            
            print(f"[Worker: {self.worker_id}] Container {name} created successfully with ID: {container.id[:12]}")
            
            # Initialize reference count
            with self._container_lock:
                self._container_refs[name] = 1
    ...
    def wait_for_healthy(self, container_name: str, timeout: int = 1800) -> bool:
        """
        ...
        Returns:
            True if container became healthy, False if timeout or not found
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                print(f"[Worker: {self.worker_id}] Container {container_name} not found during health check - failing immediately")
                # Try to get exit status for debugging
                try:
                    result = subprocess.run(['docker', 'inspect', container_name, '--format={{.State.ExitCode}} {{.State.OOMKilled}}'], 
                                          capture_output=True, text=True)
                    if result.returncode == 0 and result.stdout.strip():
                        print(f"  Container exit info: {result.stdout.strip()}")
                except:
                    pass
                # Container not found is a permanent failure - don't wait 30 minutes
                return False
            except Exception as e:
    ...
    def get_container_name(self, service: str) -> str:
        """
        Get unique container name for a service with timestamp for worker isolation.
        
        Args:
            service: Service name
            
        Returns:
            Worker-prefixed container name with timestamp suffix
        """
        import time
        timestamp = str(int(time.time()))[-6:]  # Last 6 digits of timestamp
        # Use container_prefix to ensure consistency with cleanup checks
        # This prevents containers from being marked as belonging to different workers
    ...
    def release_container(self, container_or_name) -> None:
        """
        Release a container by decrementing its reference count.
        
        Container will only be stopped/removed when reference count reaches zero.
        
        Args:
            container_or_name: Container instance or name
        """
        if isinstance(container_or_name, str):
            container_name = container_or_name
        else:
            container_name = getattr(container_or_name, 'name', str(container_or_name))
        
        with self._container_lock:
            if container_name in self._container_refs:
                self._container_refs[container_name] -= 1
                print(f"  Released container {container_name}, ref count: {self._container_refs[container_name]}")
                
                # If reference count reaches zero, mark for cleanup
                if self._container_refs[container_name] <= 0:
                    print(f"  Container {container_name} has no more references, marking for cleanup")
                    del self._container_refs[container_name]
    ...
    def cleanup(self, force: bool = False):
        ...
        for container in self.created_containers[:]:  # Create copy to avoid modification during iteration
            ...
            try:
                ...
                # Check reference count before removal
                with self._container_lock:
                    ref_count = self._container_refs.get(container_name, 0)
                    if ref_count > 0 and not force:
                        logger.debug(f"Container {container_name} still has {ref_count} references, skipping cleanup")
                        continue
                ...
                # Remove from tracking list and reference count
                try:
                    self.created_containers.remove(container)
                    with self._container_lock:
                        if container_name in self._container_refs:
        ...
        # Clear reference counts if force cleanup
        if force:
            with self._container_lock:
        ...
        # Prune dangling resources to prevent accumulation
        if force:
            try:
                ...
                if prune_result.get('ContainersDeleted'):
                    logger.info(f"Pruned {len(prune_result['ContainersDeleted'])} containers "
                               f"({prune_result.get('SpaceReclaimed', 0)} bytes reclaimed)")
                
                # Prune unused networks
                net_result = self.client.networks.prune(
                    filters={'label': [f'com.wildfire.worker={self.worker_id}']}
                )
                if net_result.get('NetworksDeleted'):
                    logger.info(f"Pruned {len(net_result['NetworksDeleted'])} networks")
            except Exception as e:
                logger.debug(f"Error during resource pruning: {e}")
...
def create_pump_controller_with_config(test_env: Dict[str, str], mqtt_conn_params: Dict[str, Any], 
                                      topic_prefix: str, auto_connect: bool = False) -> 'PumpController':
    """
    Create a PumpController instance with test-specific configuration.
    
    This function replaces the old update_gpio_config_for_tests pattern by creating
    a PumpController with proper dependency injection of configuration.
    
    Args:
        test_env: Dictionary of test environment variables
        mqtt_conn_params: MQTT connection parameters from test broker
        topic_prefix: Topic prefix for test isolation
        auto_connect: Whether to auto-connect to MQTT (default False for tests)
    ...
    Example:
        test_env = {
            ...
            "IGNITION_START_DURATION": "0.3",
            "VALVE_PRE_OPEN_DELAY": "0.1",
            # ... other env vars
        }
        conn_params = test_mqtt_broker.get_connection_params()
        controller = create_pump_controller_with_config(test_env, conn_params, topic_prefix)
    ...
    os.environ['MQTT_BROKER'] = mqtt_conn_params['host']
    os.environ['MQTT_PORT'] = str(mqtt_conn_params['port'])
    ...
    # Create configuration object which will read from environment
    config = PumpControllerConfig()
    
    # Create and return controller with injected configuration
    return PumpController(config=config, auto_connect=auto_connect)
...
def wait_for_mqtt_connection(controller: 'PumpController', timeout: float = 10.0) -> bool:
    ...
    Since PumpController doesn't have a wait_for_connection method like MQTTService,
    this helper function polls the internal MQTT client state to verify connection.
    
    Args:
        controller: PumpController instance
    ...
    Raises:
        RuntimeError: If controller doesn't have expected MQTT client attributes
    ...
    # Verify controller has MQTT client
    if not hasattr(controller, 'client'):
        raise RuntimeError("PumpController doesn't have client attribute")
    
    start_time = time.time()
    
    while time.time() - start_time < timeout:
        # First check the _mqtt_connected flag if it exists
        ...
        try:
            ...
            # In case client doesn't have is_connected method, 
            # check the internal paho client state
            try:
                if controller.client and hasattr(controller.client, '_state'):
                    # Paho MQTT client state constants
                    # mqtt_cs_connected = 3
                    if controller.client._state == 3:
                        return True
            except:
                pass
